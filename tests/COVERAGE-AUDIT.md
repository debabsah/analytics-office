...
| **Banked behavioral run** | BEHAVIORAL.md kb-reconcile block + the private design archive | an actual RED/GREEN transcript — **the only real evidence** |
...
evidence beyond kb-reconcile lives in the project's private design archive, organized by *cross-cutting
...
the private design archive's verdicts. Numbers (breadth ratios, routing counts) are countable from
