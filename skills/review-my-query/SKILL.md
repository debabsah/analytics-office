...
3. **Run the engine (two layers)** — see `references/failure-modes.md`:
   ...
   - **Failure-mode taxonomy:** hunt the classic defects — grain/cardinality, filter/context, NULL/type, time, set logic, dimensional/SCD, security/scope, determinism.
...
5. **Emit + escalate** — write `query-review.md` (template in `references/query-review.md`). If a `knowledge-base/` exists, escalate every Blocking finding to `open-questions.md`, route definition gaps back toward `kpi-contract`, and append `timeline.md`. **No KB? Write the one artifact and keep the routing notes inside it** (`groundwork` can stand up a KB so the next review builds on this). Then stop.
