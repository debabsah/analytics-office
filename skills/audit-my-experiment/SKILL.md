...
description: Use when a finished thing — a source, a result, code, or the record — is about to be trusted or consumed; the gate fires before the work leans on it. An experiment / A-B test / causal result is about to drive a decision - ship, roll out, shift budget - including when someone just wants the win written up. Switches to audit-mode and COMPUTES the validity checks a consumption read eyeballs past - sample-ratio mismatch, peeking, multiple comparisons, power/MDE, Simpson's, novelty, metric-vs-proxy - via a tested kit on the summaries in hand; missing data becomes the exact check you run and paste back. Detects: "validate this experiment", "is this A/B result real", "should we ship this test", "did the test pass", "write up our experiment win", "should we roll this out". Within this family: a forecast / time-series projection is audit-my-forecast; the SQL behind the metric is review-my-query; unvalidated source premises are audit-my-assumptions. Read-only on your data: never connects to a live system or raw data.
