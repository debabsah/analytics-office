---
name: map-my-estate
description: Use when a BI/analytics professional wants the estate DRAWN — "draw the ER diagram", "map the data flow / lineage", "show how these tables connect", "diagram how X flows from source to dashboard" — as a picture that can be trusted. Projects the knowledge base and provided code/docs into mermaid views (ER, lineage) where EVERY edge cites its evidence; an edge nothing on hand supports renders dashed [unverified], and unknowns are first-class nodes — the map doubles as a coverage picture of what you actually know. Scope-gated (map WHAT? ~25 nodes per view) and staleness-aware (each map records what it was derived from; refresh = re-run). Detects: "ER diagram", "data flow diagram", "map the lineage", "how do these tables connect". groundwork BUILDS the record — this projects it; model-contract designs FUTURE structure; review-my-query reviews the code. Never invents an edge.
